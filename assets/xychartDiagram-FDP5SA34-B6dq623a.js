...
import { l as zt } from './linear-jDizL8xw.js';
      ...
      parse: a(function (h) {
        ...
        function Vt() {
          ...
          return (
            (V = d.pop() || T.lex() || Mt),
            typeof V != 'number' &&
              (V instanceof Array && ((d = V), (V = d.pop())), (V = u.symbols_[V] || V)),
            V
          );
        }
        a(Vt, 'lex');
...
Expecting ` +
          ...
          if (B[0] instanceof Array && B.length > 1)
            throw new Error(
              'Parse Error: multiple actions possible at state: ' + H + ', token: ' + M
            );
          switch (B[0]) {
            case 1:
              (x.push(M),
            ...
            case 2:
              if (
                ...
                ci && (q._$.range = [r[r.length - (F || 1)].range[0], r[r.length - 1].range[1]]),
                (pt = this.performAction.apply(q, [f, It, lt, Y.yy, B[1], w, r].concat(li))),
                typeof pt < 'u')
              )
                return pt;
              (F &&
                ((x = x.slice(0, -1 * F * 2)), (w = w.slice(0, -1 * F)), (r = r.slice(0, -1 * F))),
...
` +
        ...
        next: a(function () {
          ...
          for (var w = this._currentRules(), r = 0; r < w.length; r++)
            if (
              ((x = this._input.match(this.rules[w[r]])), x && (!u || x[0].length > u[0].length))
            ) {
...
` + e
  ...
  Oi = { draw: zi },
  Ui = { parser: Ti, db: Wi, renderer: Oi };
