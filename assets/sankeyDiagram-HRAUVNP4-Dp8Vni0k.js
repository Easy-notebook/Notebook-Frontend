  ...
  function P(n, f, u) {
    for (let c = 1, r = n.length; c < r; ++c) {
      ...
      for (const w of k) {
        let L = 0,
          F = 0;
        for (const { source: H, value: et } of w.targetLinks) {
          let Y = et * (w.layer - H.layer);
          ((L += j(H, w) * Y), (F += Y));
        }
        if (!(F > 0)) continue;
        let U = (L / F - w.y0) * f;
  ...
  function B(n, f, u) {
    for (let c = n.length, r = c - 2; r >= 0; --r) {
      ...
      for (const w of k) {
        let L = 0,
          F = 0;
        for (const { target: H, value: et } of w.sourceLinks) {
          let Y = et * (H.layer - w.layer);
          ((L += I(w, H) * Y), (F += Y));
        }
        if (!(F > 0)) continue;
        let U = (L / F - w.y0) * f;
...
ot.prototype = kt.prototype = {
  ...
  bezierCurveTo: function (t, e, i, a, h, d) {
    this._ +=
      'C' + +t + ',' + +e + ',' + +i + ',' + +a + ',' + (this._x1 = +h) + ',' + (this._y1 = +d);
  },
        ...
        for (var A, j, I, n, f = {}, u, c, r, k; ; ) {
          if (
            ...
            this.defaultActions[j]
              ? (I = this.defaultActions[j])
              : ((A === null || typeof A > 'u') && (A = E()), (I = b[j] && b[j][A])),
...
Expecting ` +
          ...
          if (I[0] instanceof Array && I.length > 1)
            throw new Error(
              'Parse Error: multiple actions possible at state: ' + j + ', token: ' + A
            );
          switch (I[0]) {
            case 1:
              (l.push(A),
...
var K = at,
  ...
  le = p(function (t, e, i, a) {
    ...
    let _;
    h === 'sandbox' && (_ = X('#i' + e));
    const s = h === 'sandbox' ? X(_.nodes()[0].contentDocument.body) : X('body'),
      o = h === 'sandbox' ? s.select(`[id="${e}"]`) : X(`[id="${e}"]`),
...
`
  ...
  ye = K.parse.bind(K);
K.parse = (t) => ye(ue(t));
var me = { styles: fe, parser: K, db: oe, renderer: ce };
