...
export interface ExecutionStep {
  ...
  updated_steps?: any[];
  stage_id?: string;
  shotType?: string; // Backend-provided shotType: 'action'=code, 'markdown'=text
  // New fields for update-focus action
  level?: 'stages' | 'steps' | 'behaviors';
  focus?: string;
  outputs?: { expected: string[]; produced: string[]; in_progress: string[] };
  state?: any; // State to sync from backend
}
...
const ACTION_TYPES = {
  ...
  UPDATE_FOCUS: 'update-focus', // Update hierarchical focus in state machine
  COMMENT_RESULT: 'comment-result', // Add comment to result
  UPDATE_LAST_TEXT: 'update_last_text', // Update last text cell
  SET_EFFECT_THINKING: 'set_effect_as_thinking', // Set effect as thinking
  NEW_STEP: 'new_step', // New step marker
};
...
export const useScriptStore = create<ScriptStore>((set, get) => ({
  ...
  addAction: (action: Partial<ScriptAction>, couldVisibleInWritingMode = true): string | null => {
    ...
    try {
      ...
      const newAction: ScriptAction = {
        ...
        ...(action.useWorkflowThinking !== undefined && {
          useWorkflowThinking: action.useWorkflowThinking,
        }),
      ...
      set((state) => {
        const updatedStepActionIds = [
          ...(state.steps[currentStepId || '']?.actionIds || []),
          actionId,
        ];
        return {
  ...
  updateAction: (actionId: string, updates: Partial<ScriptAction> = {}): boolean => {
    try {
      ...
      if (updates.metadata) {
        useNotebookStore
          .getState()
          .updateCellMetadata(actionId, { ...notebookCell.metadata, ...updates.metadata });
      }
  ...
  finishThinking: (): void => {
    ...
    set((state) => ({
      actions: state.actions.filter((action) => action.id !== lastThinkingCell.id),
    }));
  },
    ...
    set((state) => {
      const updatedActions = state.actions.map((shot) => {
        if (shot.id === lastCell.id) {
          return {
            ...shot,
            metadata: { ...shot.metadata, finished_thinking: true, thinkingText: thinkingText },
          };
        }
        return shot;
      });
    ...
    try {
      ...
      if (result?.success && result.outputs) {
        ...
        if (Array.isArray(result.outputs)) {
          ...
          const effectText =
            result.outputs.content || result.outputs.text || result.outputs.toString();
    ...
    try {
      switch (actionType) {
        ...
        case ACTION_TYPES.EXEC_CODE: {
          const targetId =
            step.codecell_id === 'lastAddedCellId' ? get().lastAddedActionId : step.codecell_id;
          if (targetId) {
            ...
            globalUpdateInterface.createAIRunningCode(
              `Execution completed`,
              '',
              [],
              targetId,
            ...
            globalUpdateInterface.createSystemEvent(
              'Execution failed: Cell ID not found',
              '',
              [],
              null
            );
        ...
        case ACTION_TYPES.COMPLETE_STEP: {
          ...
          console.log(
            `[useScriptStore] Received command to complete step. Transitioning with ${EVENTS.COMPLETE_STEP}.`
          );
        ...
        case ACTION_TYPES.UPDATE_WORKFLOW: {
          ...
          if (!workflowData || !workflowData.workflowTemplate) {
            console.error(
              '[useScriptStore] UPDATE_WORKFLOW action received without valid workflow data:',
              {
                step,
          ...
          globalUpdateInterface.createSystemEvent(
            'Workflow update received, awaiting user confirmation.',
            '',
            []
          );
          syncStateIfPresent();
        ...
        case ACTION_TYPES.UPDATE_STEP_LIST: {
          ...
          if (step.updated_steps && step.stage_id) {
            ...
            globalUpdateInterface.createSystemEvent(
              `Workflow stage '${step.stage_id}' has been updated.`,
              '',
              []
            );
          } else {
            console.error(
              '[useScriptStore] UPDATE_STEP_LIST action is missing stage_id or updated_steps.'
            );
          }
          syncStateIfPresent();
          break;
        }
        case ACTION_TYPES.UPDATE_FOCUS: {
          // Update hierarchical focus in state machine
          if (!step.level || !step.focus) {
            console.error('[useScriptStore] UPDATE_FOCUS action requires level and focus');
            break;
          }
          console.log(`[useScriptStore] Updating ${step.level} focus (${step.focus.length} chars)`);
          const { updateProgressFocus } = useWorkflowStateMachine.getState();
          updateProgressFocus(step.level, step.focus);

          // Optionally update outputs if provided
          if (step.outputs) {
            const { updateProgressOutputs } = useWorkflowStateMachine.getState();
            updateProgressOutputs(step.level, step.outputs);
          }
          syncStateIfPresent();
          break;
        }
        case ACTION_TYPES.COMMENT_RESULT: {
          // Add comment/reflection to the last cell
          if (step.content) {
            const actionId = step.storeId || uuidv4();
            get().addAction({
              id: actionId,
              type: 'text',
              content: step.content,
              metadata: { ...step.metadata, isComment: true },
            });
          }
          syncStateIfPresent();
          break;
        ...
        case ACTION_TYPES.SET_EFFECT_THINKING: {
          get().setEffectAsThinking(step.thinkingText);
          syncStateIfPresent();
          break;
        }
        case ACTION_TYPES.NEW_STEP: {
          // New step marker (similar to new chapter but for steps)
          const actionId = step.storeId || uuidv4();
          get().addAction({
            id: actionId,
            type: 'text',
            content: `### ${step.content || 'New Step'}`,
            metadata: { ...step.metadata, isStep: true },
          });
          syncStateIfPresent();
          break;
        }
