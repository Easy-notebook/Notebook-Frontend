...
const PipelineStageWrapper: React.FC<PipelineStageWrapperProps> = ({
  ...
  if (isAnimating) {
    if (stage === currentStage) {
      ...
      animationClass =
        animationDirection === 'next' ? 'animate-slide-up-out' : 'animate-slide-down-out';
    } else {
      // New stage is animating in
      animationClass =
        animationDirection === 'next' ? 'animate-slide-up-in' : 'animate-slide-down-in';
    }
  }

  return <div className={`w-full h-full ${animationClass}`}>{children}</div>;
};
