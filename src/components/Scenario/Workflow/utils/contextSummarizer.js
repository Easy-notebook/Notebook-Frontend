...
export class ContextSummarizer {
  ...
  compressHistory(history, maxItems = this.maxHistoryItems) {
    ...
    // 保留里程碑
    const milestones = history.filter((item) => item.is_milestone || item.confidence_level > 0.8);
    ...
    const unique = combined.filter(
      (item, index, arr) => arr.findIndex((t) => t.timestamp === item.timestamp) === index
    );
  ...
  optimizeContextForTransmission(context) {
    ...
    if (optimized.agent_thinking_history) {
      optimized.agent_thinking_history = this.compressHistory(optimized.agent_thinking_history);
    }
