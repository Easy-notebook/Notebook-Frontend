...
export const usePipelineStore = create<PipelineStore>((set) => ({
  ...
  setAnimation: (animating, direction = 'forward') =>
  ...
  initializeWorkflow: async (planningRequest) => {
    ...
    try {
      ...
      console.log(
        '[PipelineStore] Predefined workflow template set successfully. Ready for user confirmation.'
      );
    } catch (error) {
  ...
  updateStepsForStage: (stageId, newSteps) => {
    set((state) => {
      if (!state.workflowTemplate) {
        console.warn(
          `[PipelineStore] Cannot update steps for stage '${stageId}': workflowTemplate is null.`
        );
        return state;
      }
