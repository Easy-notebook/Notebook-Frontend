...
export const useCellManager = (options: CellManagerOptions = {}) => {
  ...
  const insertCell = useCallback(
    (type: string, index?: number, content = '') => {
      const newCell = {
        id: generateCellId(),
        type,
        content,
  ...
  // 在当前选中cell之后插入新cell
  const insertCellAfterCurrent = useCallback(
    (type: string, content = '') => {
      if (selectedCellId) {
  ...
  // 在当前选中cell之前插入新cell
  const insertCellBeforeCurrent = useCallback(
    (type: string, content = '') => {
      if (selectedCellId) {
        const currentIndex = getCellIndex(selectedCellId);
  ...
  const updateCellContent = useCallback(
    (cellId: string, content: string) => {
      const newCells = cells.map((cell) => (cell.id === cellId ? { ...cell, content } : cell));
      setCells(newCells);
    },
    [cells, setCells]
  );
