...
function createTableFromHeader(view, headerText) {
  ...
  const headerCells = headers.map((header) =>
    schema.nodes.tableHeader.create(null, schema.nodes.paragraph.create(null, schema.text(header)))
  );
...
function addTableRow(view) {
  ...
  for (let i = 0; i < cellCount; i++) {
    newCells.push(schema.nodes.tableCell.create(null, schema.nodes.paragraph.create()));
  }
      ...
      if ($newPos.parent.type.name === 'table') {
        ...
        for (let i = newPos; i < tr.doc.content.size; i++) {
          ...
          if (
            ...
            $pos.parent.parent?.type.name === 'tableCell'
          ) {
            newPos = i;
            break;
          }
