...
const JupyterNotebookEditor = forwardRef<JupyterNotebookEditorHandle, JupyterNotebookEditorProps>(
  ({ className = '', readOnly = false }, ref) => {
    ...
    /* ------------------------- Type helpers ------------------------- */
    type RendererOutput = {
      type: 'error' | 'image' | 'text' | 'html';
      content: unknown;
      key?: string;
      timestamp?: string;
    };
    const normalizeOutputsForRenderer = useCallback(
      (outs?: OutputItem[] | unknown[]): RendererOutput[] => {
        const arr = Array.isArray(outs) ? outs : [];
        return arr.map((o: unknown): RendererOutput => {
          const output = o as {
            type?: string;
            content?: unknown;
            key?: string;
          ...
          const t = output.type;
          const normalizedType: RendererOutput['type'] =
            t === 'error' || t === 'image' || t === 'html' ? t : 'text';
          return {
            type: normalizedType,
    ...
    const ensureHTMLElement = (el: Element | null): el is HTMLElement =>
      !!el && el instanceof HTMLElement;

    const isCellType = (val: unknown): val is CellType =>
      typeof val === 'string' &&
      ['code', 'markdown', 'raw', 'hybrid', 'image', 'thinking', 'link'].includes(val);
    ...
    const handleCellsReorder = useCallback(
      (newCells: unknown[]) => {
    ...
    const handleAddCell = useCallback(
      ...
      [generateCellId, setCells, cells]
    );

    // Adapter for DnD list onAddCell signature
    const handleAddCellForDnD = useCallback(
    ...
    const renderCell = useCallback(
      (cell: Cell) => {
        ...
        const commonProps = {
          ...
          onMoveDown:
            cellIndex < cells.length - 1 ? () => handleMoveCell(cell.id, 'down') : undefined,
    ...
    /* ------------------------ Click-to-focus ------------------------ */
    const handleEditorClick: React.MouseEventHandler<HTMLDivElement> = useCallback(
      (e) => {
        ...
        <div className="flex-1 w-full max-w-screen-lg mx-auto px-8 lg:px-16 flex flex-col">
          ...
          <div className="relative flex-shrink-0">
            <DraggableCellList
              ...
              renderCell={(c: unknown, isDragging?: boolean) => {
