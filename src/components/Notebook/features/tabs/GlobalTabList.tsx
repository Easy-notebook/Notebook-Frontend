...
const GlobalTabList: React.FC = () => {
  ...
  const { currentPreviewFiles, activeFile, isTabDirty, previewMode, getCurrentNotebookId } =
    usePreviewStore();
  ...
  const handleTabClose = useCallback(
    (tabId: string) => {
      ...
      const remainingFileTabs = currentPreviewFiles.filter(
        (file) => file.id !== tabId && file.id.startsWith(`${currentNotebookId}::`)
      );
  ...
  // 显示规则：
  // 1. 只在 workspace 路由下显示
  // 2. 当只有一个 Notebook tab（没有其他文件 tabs）时不显示
  const currentNotebookId = getCurrentNotebookId();
  const { currentView } = useRouteStore();
  const shouldShowTabs = !!currentNotebookId && currentView === 'workspace' && tabs.length > 1;
