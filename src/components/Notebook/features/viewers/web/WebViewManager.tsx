...
const WebViewManager: React.FC<WebViewManagerProps> = ({
  ...
  function detectProjectType(
    path: string
  ): 'html' | 'react' | 'javascript' | 'typescript' | 'vue' | 'angular' {
        ...
        {isLoadingProjects ? (
          ...
          <div className="p-4 space-y-2">
            {availableProjects.map((project) => (
              <ProjectCard key={project.path} project={project} onSelect={selectProject} />
            ))}
          </div>
        ) : (
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Content */}
      <div className="flex-1">{isProjectBrowser ? renderProjectBrowser() : renderWebView()}</div>
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div className="mt-0.5">{getProjectIcon()}</div>
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500">
            ...
            {project.size && <span>{(project.size / 1024).toFixed(1)} KB</span>}
