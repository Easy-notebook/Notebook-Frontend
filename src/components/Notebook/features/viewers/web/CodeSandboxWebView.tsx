    ...
    try {
      ...
      if (projectType === 'html' && Object.keys(config.files).some((path) => path.includes('/'))) {
        ...
        if (notebookIndex !== -1) {
          ...
          // 使用本地静态服务提供完整多文件项目
          const derived =
            deriveSandboxUrlFromProjectPath(projectPath) ||
            `/sandbox/${notebookId}/${projectName}/`;
      ...
      <div className="flex-1 relative">
        {error ? (
          ...
          <div className="flex flex-col items-center justify-center h-full p-8">
            ...
            <div className="text-sm text-gray-500">Scanning {projectType} project structure</div>
          </div>
        ) : sandboxUrl ? (
          <iframe
            ref={iframeRef}
