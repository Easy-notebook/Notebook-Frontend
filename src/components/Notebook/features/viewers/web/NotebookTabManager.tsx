...
const NotebookTabComponent: React.FC<NotebookTabProps> = ({
  tab,
  onSelect,
  onClose,
  onRename,
...
const NotebookTabManager: React.FC = () => {
  ...
  const handleTabRename = useCallback((tabId: string, newName: string) => {
    setTabs((prev) =>
      prev.map((tab) => (tab.id === tabId ? { ...tab, name: newName, isDirty: true } : tab))
    );
