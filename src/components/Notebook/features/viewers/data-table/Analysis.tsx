  ...
  static analyzeData(data: CSVRow[]): CSVAnalytics {
    ...
    columns.forEach((column) => {
      ...
      const isMostlyNumeric =
        numericValues.length >= values.length * 0.8 && numericValues.length > 0;
      const isMostlyDate =
        !isMostlyNumeric && dateValues.length >= values.length * 0.8 && dateValues.length > 0;

      const base: ColumnStats = {
        type: isMostlyNumeric
          ? 'number'
          : isMostlyDate
            ? 'date'
            : values.length
    ...
    return {
      ...
      memoryUsage: approxSizeKB(data),
      columnStats,
      dataQuality: {
        completeness: Math.round(completeness * 100) / 100,
        consistency: Math.max(0, Math.min(100, 100 - (nullCells / Math.max(1, totalCells)) * 120)),
        validity: 90,
      },
    };
  }
      ...
      if (fileType === 'xlsx') {
        ...
        for (const seg of segments) {
          ...
          const colW = headers.map((h, i) =>
            pickColumnWidth(colsMeta[i]?.wpx, String(headersRaw[i] ?? '').length)
          );
        ...
        if (looksJson) {
          ...
          outBlocks.push({
            headers,
            headersDisplay: headersRaw,
            rows,
            colWidths: headers.map((h) => pickColumnWidth(undefined, h.length)),
          });
    ...
    return (
      <div className="p-6 bg-white rounded-lg">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-theme-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <BarChart3 className="w-5 h-5 text-theme-600" />
              <h3 className="font-semibold text-theme-900">Total Rows</h3>
            </div>
            <p className="text-2xl font-bold text-theme-700">
          ...
          <div className="bg-green-50 p-4 rounded-lg">
            ...
            <p className="text-2xl font-bold text-green-700">{analytics.totalColumns}</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <TrendingUp className="w-5 h-5 text-purple-600" />
              <h3 className="font-semibold text-purple-900">Completeness</h3>
            </div>
            <p className="text-2xl font-bold text-purple-700">
              {analytics.dataQuality.completeness}%
            </p>
          </div>
          <div className="bg-orange-50 p-4 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <MoreHorizontal className="w-5 h-5 text-orange-600" />
              <h3 className="font-semibold text-orange-900">Memory Usage</h3>
            </div>
            <p className="text-2xl font-bold text-orange-700">{analytics.memoryUsage}</p>
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {Object.entries(analytics.columnStats).map(([column, stats]) => (
                <tr key={column} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-theme-700">
                    {column}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    <span
                      className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${stats.type === 'number' ? 'bg-theme-100 text-theme-800' : stats.type === 'date' ? 'bg-indigo-100 text-indigo-800' : 'bg-amber-100 text-amber-800'}`}
                    >
                      {stats.type === 'number'
                        ? 'Numeric'
                        : stats.type === 'date'
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    {analytics.totalRows > 0
                      ? (
                          ((analytics.totalRows - stats.nullCount) / analytics.totalRows) *
                          100
                        ).toFixed(1)
                      : 0}
                    %
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                    ...
                    {analytics.totalRows > 0 && (
                      <span className="ml-1 text-gray-500 text-xs">
                        (
                        {(
                          (stats.uniqueCount / Math.max(1, analytics.totalRows - stats.nullCount)) *
                          100
                        ).toFixed(1)}
                        %)
                      </span>
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center text-gray-400">
          <div className="text-lg mb-2">No CSV/Excel file selected</div>
          <div className="text-sm">Select a CSV or Excel file to preview</div>
        </div>
      </div>
    );
  }
                    ...
                    {activeBlock && (
                      <>
                        ...
                        <td className="pr-6 text-sm font-medium align-middle">
                          {(activeBlock.rows?.length ?? 0).toLocaleString()}
                        </td>
                        <td className="pr-6 align-middle">
                          <span className="inline-flex items-center rounded-md bg-theme-600 px-2 py-1 text-xs font-medium text-white">
                            Columns
                  ...
                  >
                    {blocks.map((b, i) => (
                      <option
                        key={i}
                        value={i}
                      >{`Table ${i + 1} — ${b.headers.length} cols, ${b.rows.length} rows`}</option>
                    ))}
                  </select>
                </div>
              )}
