...
const DocDisplay: React.FC<DocDisplayProps> = ({
  ...
  const loadDocument = useCallback(async () => {
    try {
      ...
      console.log(
        'DocDisplay - fileContent length:',
        typeof fileContent === 'string' ? fileContent.length : fileContent.byteLength
      );
      console.log(
        'DocDisplay - fileContent preview:',
        typeof fileContent === 'string' ? fileContent.substring(0, 100) + '...' : '[ArrayBuffer]'
      );
      ...
      if (typeof fileContent === 'string') {
        ...
        // Check if it's a URL that we need to fetch
          ...
          fileContent.startsWith('https://') ||
          fileContent.startsWith('/assets/')
        ) {
          ...
          if (!base64Regex.test(base64Data)) {
            ...
            throw new Error(
              'Invalid base64 format: contains invalid characters or is not base64 encoded'
            );
          }
      ...
      setError(
        `Document loading failed: ${errorMessage}. Please ensure the file is a valid DOC/DOCX document.`
      );
    } finally {
      setIsLoading(false);
    }
