...
// moved to sections/LeftSideBar/Main/KnowledgeForest
...
const KnowledgeForestSidebar = ({
  ...
  const [filterType, setFilterType] = useState<
    'all' | 'markdown' | 'code' | 'hybrid' | 'link' | 'image'
  >('all');
  ...
  const renderBottomSection = useCallback(() => {
    return (
      <div className="w-full h-12 px-4 flex items-center justify-between border-t border-gray-200 bg-gray-50">
        ...
        <span className="text-xs text-gray-500">{notebookTitle || 'Untitled Notebook'}</span>
      </div>
    );
                    ...
                    {filteredRecords.map((record) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className="flex-shrink-0 mt-0.5">{getTypeIcon(record.type)}</div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="text-xs text-gray-500 capitalize">{record.type}</div>
                            </div>
                          </div>
                        </div>
                      </div>
