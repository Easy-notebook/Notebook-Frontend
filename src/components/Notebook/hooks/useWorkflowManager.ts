...
export const useWorkflowManager = (
  ...
  const handleConfirmWorkflowUpdate = useCallback(() => {
    ...
    const { confirmWorkflowUpdate, pendingWorkflowUpdate: fsmPending } =
  ...
  const processWorkflowUpdate = useCallback(
    (workflow: any) => {
  ...
  const processStageStepsUpdate = useCallback(
    (
      ...
      if (updateStageId === stageId) {
        ...
        if (currentWorkflow && currentWorkflow.stages) {
          const updatedWorkflow = {
            ...
            stages: currentWorkflow.stages.map((stage) =>
              stage.id === updateStageId ? { ...stage, steps: updatedSteps } : stage
            ),
          };
        ...
        console.log(
          `Stage steps update for different stage: ${updateStageId}, current: ${stageId}`
        );
      }
    },
    [stageId]
