      ...
      <div className="h-full flex flex-col bg-white">
        ...
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center space-x-3">
            <h2 className="text-lg font-semibold" style={{ color: '#35495E' }}>
              {agentProfile.name}
            </h2>
          </div>
          {onBack && (
          ...
          <div className="max-w-2xl mx-auto space-y-8 items-center justify-center">
            ...
            <div>
              ...
              <div
                ...
                style={{ color: '#35495E', lineHeight: '1.6' }}
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                {agentProfile.capabilities.slice(0, 4).map((capability, idx) => (
                  <Card
                    ...
                    style={{ background: 'rgba(65, 184, 131, 0.03)' }}
                  >
                    <CardContent className="p-3">
                      <div className="text-xs font-medium mb-1" style={{ color: '#41B883' }}>
                        {capability.icon}
                      </div>
                      <div className="text-sm font-medium" style={{ color: '#35495E' }}>
                        {capability.name}
                      </div>
                    </CardContent>
                  </Card>
            ...
            <Card style={{ background: 'rgba(65, 184, 131, 0.02)' }}>
              <CardContent className="flex items-center justify-between py-3 px-4">
                ...
                <div className="text-center">
                  ...
                  <div className="text-xs" style={{ color: '#6B7280' }}>
                    {agentProfile.specialty.length} areas
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="text-xs" style={{ color: '#6B7280' }}>
                    {agentProfile.modes.filter((m) => m.enabled).length} active
                  </div>
                </div>
              </CardContent>
            </Card>
  ...
  return (
    <div className="h-full flex flex-col bg-white">
      ...
      <div
        ...
        style={{ borderColor: 'rgba(65, 184, 131, 0.1)' }}
      >
        <div className="flex items-center space-x-3">
          <div
            className={`w-8 h-8 rounded-full ${agentProfile.color} flex items-center justify-center`}
          >
          ...
          <div>
            ...
            <div className="text-xs" style={{ color: '#6B7280' }}>
              Active since {formatTimestamp(agentMemory.created_at)} • Last updated{' '}
              {formatTimestamp(agentMemory.last_updated)}
            </div>
          </div>
        </div>
        {onBack && (
      ...
      <div
        ...
        style={{ background: 'rgba(65, 184, 131, 0.02)', borderColor: 'rgba(65, 184, 131, 0.1)' }}
      >
            ...
            <div className="text-center">
              <div className="font-semibold" style={{ color: '#F59E0B' }}>
                {agentMemory.termination_conditions?.current_counts?.debug_cycles ||
                  ...
                  agentMemory.termination_conditions?.current_counts?.question_rounds ||
                  0}
              </div>
              <div className="text-xs" style={{ color: '#6B7280' }}>
                Cycles
      ...
      <div className="flex-1 overflow-y-auto px-6">
        <div className="max-w-2xl mx-auto space-y-6 py-4">
          ...
          {agentMemory.user_intent_observations?.progress_markers?.current_focus && (
            <div>
              <h3 className="text-sm font-medium mb-2" style={{ color: '#35495E' }}>
                Current Focus
              </h3>
              <Card style={{ background: 'rgba(52, 144, 220, 0.05)' }}>
                <CardContent className="p-3 text-sm" style={{ color: '#35495E' }}>
                  {agentMemory.user_intent_observations.progress_markers.current_focus}
                </CardContent>
              </Card>
            </div>
          ...
          {agentMemory.user_intent_observations?.stated_goals?.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {agentMemory.user_intent_observations.stated_goals
                  .slice(0, 2)
                  .map((goal: string, idx: number) => (
                    <div
                      key={idx}
                      className="flex items-start space-x-3 p-2 rounded"
                      style={{ background: 'rgba(65, 184, 131, 0.02)' }}
                    >
                      <span
                        className="w-1.5 h-1.5 rounded-full mt-2 flex-shrink-0"
                        style={{ background: '#41B883' }}
                      ></span>
                      <span className="text-sm" style={{ color: '#35495E' }}>
                        {goal}
                      </span>
                    </div>
                  ))}
          ...
          {agentMemory.learned_patterns?.user_preferences && (
            <div>
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                {agentMemory.learned_patterns.user_preferences.preferred_libraries
                  ?.slice(0, 4)
                  .map((lib: string, idx: number) => (
                    <span
                      key={idx}
            ...
            <div>
              ...
              <div className="space-y-2">
                {agentMemory.interactions.slice(0, 2).map((interaction: any, idx: number) => (
                  <Card key={idx} style={{ background: 'rgba(65, 184, 131, 0.02)' }}>
                    <CardContent className="p-3">
                      <div className="flex items-center justify-between mb-1">
                        <span
                          className={`text-xs px-2 py-0.5 rounded ${getStatusBadge(interaction.outcome)}`}
                        >
                          {interaction.outcome}
                        </span>
                      ...
                      <div className="text-xs" style={{ color: '#35495E' }}>
                        {interaction.user_input?.slice(0, 100)}
                        {interaction.user_input?.length > 100 && '...'}
                      </div>
                    </CardContent>
                  </Card>
                ))}
                  ...
                  {agentMemory.situation_tracking.task_completion.completed_requirements?.length >
                    ...
                    <div className="text-center">
                      <div className="text-sm font-semibold" style={{ color: '#41B883' }}>
                        {
                          agentMemory.situation_tracking.task_completion.completed_requirements
                            .length
                        }
                      </div>
                      <div className="text-xs" style={{ color: '#6B7280' }}>
          ...
          {agentMemory.termination_conditions && (
            <div>
              ...
              <Card style={{ background: 'rgba(52, 144, 220, 0.02)' }}>
                <CardContent className="p-3">
                    ...
                    <div>
                      ...
                      <span style={{ color: '#6B7280' }}>
                        {agentProfile.maxIterations.debug_cycles ||
                          ...
                          agentProfile.maxIterations.question_rounds ||
                          'N/A'}
                      </span>
                    </div>
                  </div>
