...
const NotebookApp = () => {
  ...
  const handleLibrarySelectNotebook = useCallback(
    async (notebookId: string, notebookTitle: string) => {
      try {
      ...
      const props = {
        ...
        onDelete:
          (viewMode as any) === 'complete' || (viewMode as any) === 'create'
            ? () => deleteCell(cell.id)
            : undefined, // 直接调用 store 的 deleteCell 动作
    ...
    <div className="h-screen flex border-r border-black">
      ...
      <div className="flex-1 flex flex-col overflow-hidden relative m-0 p-0">
        ...
        <div className="flex-1 overflow-y-auto scroll-smooth border-3 border-theme-200 bg-white w-full h-full">
          ...
          <div className="w-full h-full">{resolveMainContent().component}</div>
        </div>
