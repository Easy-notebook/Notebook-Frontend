...
export class NotebookLogger extends Logger {
    ...
    if (details) {
      if (details.content) {
        builder
          .text(' Content: ')
          .code(
            details.content.substring(0, 50) + (details.content.length > 50 ? '...' : ''),
            'text'
          );
      }
      if (details.type) {
   ...
   */
  lifecycleEvent(
    event: 'create' | 'load' | 'save' | 'close' | 'delete',
    notebookId: string,
    details?: any
  ): void {
    const eventColors = {
   ...
   */
  taskManagement(
    action: 'parse' | 'execute' | 'complete' | 'skip',
    taskInfo: string,
    details?: any
  ): void {
    const actionColors = {
  ...
  validation(
    ...
    if (issues && issues.length > 0) {
      builder.text(' Issues: ').code(issues.join(', '), 'text');
    }
   ...
   */
  contentProcessing(
    operation: 'parse' | 'format' | 'validate' | 'transform',
    cellId: string,
    details?: any
  ): void {
    const operationColors = {
  ...
  stateChange(property: string, oldValue: any, newValue: any, context?: string): void {
    ...
    if (context) {
      builder.text(' Context: ').badge(context, { backgroundColor: '#6B7280', color: '#FFFFFF' });
    }
