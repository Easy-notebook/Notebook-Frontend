...
export class StoreLogger extends Logger {
  ...
  storeInit(storeName: string, initialState?: any): void {
    ...
    if (initialState) {
      builder.text(' Initial State: ').code(JSON.stringify(initialState, null, 2), 'json');
    }
  ...
  actionDispatch(storeName: string, actionName: string, payload?: any, timestamp?: number): void {
    ...
    if (payload) {
      builder.text(' Payload: ').code(JSON.stringify(payload, null, 2), 'json');
    }
  ...
  stateMutation(
    ...
    if (mutationPath) {
      builder.text(' Path: ').code(mutationPath, 'text');
    }
  ...
  computedEvaluation(
    ...
    if (evaluationTime !== undefined) {
      builder.text(' Time: ').badge(`${evaluationTime}ms`, {
        backgroundColor: evaluationTime > 10 ? '#F59E0B' : '#10B981',
        color: '#FFFFFF',
      });
    }

    builder.text(' Result: ').code(JSON.stringify(result), 'json');
  ...
  subscription(
    ...
    if (subscriberInfo) {
      builder.text(' Subscriber: ').code(subscriberInfo, 'text');
    }
  ...
  persistence(storeName: string, operation: 'save' | 'load' | 'clear', details?: any): void {
    ...
    if (details) {
      if (details.size !== undefined) {
        builder
          .text(' Size: ')
          .badge(`${(details.size / 1024).toFixed(2)}KB`, {
            backgroundColor: '#6366F1',
            color: '#FFFFFF',
          });
      }
      if (details.key) {
        builder.text(' Key: ').code(details.key, 'text');
      }
      if (details.success !== undefined) {
        builder.text(' Status: ').badge(details.success ? 'SUCCESS' : 'FAILED', {
  ...
  middleware(
    ...
    if (executionTime !== undefined) {
      builder.text(' Time: ').badge(`${executionTime}ms`, {
  ...
  stateValidation(
    ...
    if (errors && errors.length > 0) {
      builder.text(' Errors: ').code(errors.join(', '), 'text');
    }
  ...
  performance(
    ...
    if (stateSize !== undefined) {
      builder
        .text(' State Size: ')
        .badge(`${(stateSize / 1024).toFixed(2)}KB`, {
