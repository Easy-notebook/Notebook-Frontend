...
const RESERVED_SHORTCUTS = new Set(['Ctrl+C', 'Ctrl+V', 'Ctrl+X', 'Ctrl+A', 'Ctrl+Z']);
...
const storageManager = {
  ...
  async checkStorageSpace(): Promise<boolean> {
    try {
      ...
      storeLog.error('Storage space check failed', { error: e });
      return false;
    }
  },
  ...
  async clearOldBackups(): Promise<void> {
    ...
    const backups = keys
      .filter((key) => backupPattern.test(key))
      .sort((a, b) => {
  ...
  async createBackup(data: SettingsWithTimestamp): Promise<void> {
    try {
      ...
      storeLog.error('Backup creation failed', { error });
    }
  },
  ...
  async restoreFromBackup(): Promise<PersistedState | null> {
    ...
    if (latestBackup) {
      try {
        ...
        storeLog.error('Backup restoration failed', { error });
        return null;
      }
    }
    return null;
  ...
  async cleanupStorage(): Promise<boolean> {
    try {
      ...
      storeLog.error('Storage cleanup failed', { error: e });
      return false;
    }
  },
};
...
const validateShortcut = (shortcut: string, existingShortcuts: Shortcuts): boolean => {
  ...
  const conflicts = Object.entries(existingShortcuts).filter(
    ([_, value]) => value.toLowerCase().replace(/\s+/g, '') === normalizedShortcut
  );
  ...
  storage: {
    getItem: async (name: string): Promise<PersistedState | null> => {
      try {
        ...
        storeLog.error('Error reading settings', { error });
      ...
      switch (version) {
        ...
        default:
          if (version > STORE_VERSION) {
            storeLog.warn('Future version detected, resetting to initial state', {
