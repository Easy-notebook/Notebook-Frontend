...
interface GlobalUpdateInterface {
  ...
  createGeneratingVideoCell: (
    prompt: string,
    params?: { quality?: string; ratio?: string; duration?: string }
  ) => string;
...
const globalUpdateInterface: GlobalUpdateInterface = {
  ...
  addCell: (newCell: Cell, index?: number) =>
    useStore.getState().addCell(newCell as Partial<StoreCell>, index),
  ...
  updateCellOutputs: (cellId: string, outputs: any[]) =>
    useStore.getState().updateCellOutputs(cellId, outputs),
  updateCurrentCellWithContent: (content: string) =>
    useStore.getState().updateCurrentCellWithContent(content),
  ...
  setCurrentRunningPhaseId: (phaseId: string) =>
    useStore.getState().setCurrentRunningPhaseId(phaseId),
  ...
  addNewCellWithUniqueIdentifier: (
    ...
    useStore
      .getState()
      .addNewCellWithUniqueIdentifier(type, description, enableEdit, uniqueIdentifier, prompt),
  updateCellByUniqueIdentifier: (uniqueIdentifier: string, updates: Partial<any>) =>
  ...
  updateCurrentCellDescription: (description: string) =>
    useStore.getState().updateCurrentCellDescription(description),
  addNewContent2CurrentCellDescription: (content: string) =>
    useStore.getState().addNewContent2CurrentCellDescription(content),

  getHistoryCode: (): string => useStore.getState().getHistoryCode(),
  getPhaseHistoryCode: (phaseId: string): string =>
    useStore.getState().getPhaseHistoryCode(phaseId),
  ...
  addContentToAnswer: (QId: string, content: string) =>
    useAIAgentStore.getState().addContentToAnswer(QId, content),
  finishStreamingAnswer: (QId: string, response?: string) =>
    useAIAgentStore.getState().finishStreamingAnswer(QId, response),
   ...
   */
  createUserAskQuestion: (
    content: string,
    relatedQAIds: string[] = [],
    cellId: string | null = useStore.getState().getCurrentCellId()
  ) => {
  ...
  createAIExplainingProcess: (
    ...
    const action = createAIExplainingProcessAction(
      content,
      result,
      relatedQAIds,
      cellId,
