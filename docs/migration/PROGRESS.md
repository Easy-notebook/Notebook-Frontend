...
| 3 — Command registry | ✅ done | Store-free registry extended in place: `FullCommandContext` (+selection/arg/t/intents + isNodeActive/isMarkActive/currentCellId helpers), `txCommand`/`serviceCommand` factories, combinators (`first`/`sequence`(single-undo)/`when`/`tap`/`byId`), `EditorIntent`/`IntentSink`, `createBuiltinCommands()` (paragraph/h1-3/lists/quote/divider/math/code-cell/table/raw/thinking/image + bold/italic/strike/inline-code/link + nav + exec/history/save), `registry.list({surface,query,group})`/`run`/`toKeymap`. Adversarial pass: 5 findings, all blocker/major fixed (`sequence()` selection RangeError, `first()` async, `link.canRun` dead Mod-k). **153 core tests, 0 core tsc errors.** Deferred to P4/P6: `OutputPort`/`NotebookCapability`, `slashTriggerPlugin`, React menus, `nav.backspace-downgrade` wrapper traversal. Known minor: keymap dry-run probe optimistically true for service commands. |
